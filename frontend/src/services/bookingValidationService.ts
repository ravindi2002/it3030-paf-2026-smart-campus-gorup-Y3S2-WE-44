import { BookingRequest, BookingValidationResult, BookingValidationError, BookingClarification, BookingValidationRule } from '../types/Booking';
...
export class BookingValidationService {
  private static validationRules: Record<string, BookingValidationRule[]> = {
    ...
    endTime: [
      { field: 'endTime', required: true, errorCode: 'END_TIME_REQUIRED' },
      { 
        field: 'endTime', 
        custom: (value: any, context?: BookingRequest) => {
        ...
        errorCode: 'END_TIME_AFTER_START' 
      },
      { 
        field: 'endTime', 
        custom: (value: any) => {
        ...
        errorCode: 'END_TIME_BUSINESS_HOURS' 
      }
    ],
    purpose: [
      { field: 'purpose', required: true, minLength: 10, maxLength: 500, errorCode: 'PURPOSE_REQUIRED' },
      { field: 'purpose', pattern: /^[a-zA-Z0-9\s\-.,!?]+$/, errorCode: 'PURPOSE_INVALID_CHARS' }
    ],
    expectedAttendees: [
      { field: 'expectedAttendees', required: true, min: 1, max: 1000, errorCode: 'ATTENDEES_REQUIRED' },
      { 
        ...
        errorCode: 'ATTENDEES_CAPACITY' 
      }
    ],
    department: [
      { field: 'department', required: true, minLength: 2, maxLength: 100, errorCode: 'DEPARTMENT_REQUIRED' }
    ],
    eventType: [
      { field: 'eventType', required: true, errorCode: 'EVENT_TYPE_REQUIRED' }
    ],
    contactPhone: [
      { field: 'contactPhone', pattern: /^[\+]?[1-9][\d\-\s\(\)]{7,15}$/, errorCode: 'PHONE_INVALID' }
    ],
    contactEmail: [
      { field: 'contactEmail', required: true, pattern: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, errorCode: 'EMAIL_INVALID' }
    ],
    priority: [
      { field: 'priority', required: true, errorCode: 'PRIORITY_REQUIRED' }
    ]
  };
  ...
  private static clarificationQuestions: BookingClarification[] = [
    {
      ...
      validation: [
        { field: 'purpose_details', required: true, minLength: 20, maxLength: 1000, errorCode: 'PURPOSE_DETAILS_REQUIRED' }
      ]
    },
    {
    ...
    },
    {
      ...
      helpText: 'Specify recurring pattern for your booking'
    },
    {
      id: 'estimated_cost',
      ...
      validation: [
        { field: 'estimated_cost', min: 0, max: 10000, errorCode: 'COST_OUT_OF_RANGE' }
      ]
    },
    {
  ...
  static validateBooking(booking: BookingRequest): BookingValidationResult {
    ...
    // Validate each field
    Object.keys(booking).forEach((field: string) => {
      const fieldRules = this.validationRules[field];
      if (fieldRules) {
        fieldRules.forEach((rule: BookingValidationRule) => {
  ...
  static getErrorMessage(errorCode: string): string {
    const errorMessages: Record<string, string> = {
      ...
      'PURPOSE_DETAILS_REQUIRED': 'Please provide more details about the purpose',
      'COST_OUT_OF_RANGE': 'Estimated cost must be between 0 and 10000',
